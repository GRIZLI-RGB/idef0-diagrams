...
export const DiagramBlock = ({ currentDiagram, currentDiagrams, currentLevel, styles }) => {
    ...
    const checkNextDiagram = () => {
        let flag = false;
        currentDiagrams[currentLevel].map((item) => item.id === currentDiagram.id + 1 ? flag = true : null)
        return flag;
    }
    return (
                ...
                <ul className="diagramBlock__ul diagramBlock__outputs">
                    {currentDiagram.outputs.map((output, index) => (
                        <>
                            ...
                            {
                                checkNextDiagram() ? <Xarrow
                                start={`diagramBlock-${currentDiagram?.id}`}
                                end={`diagramBlock-${currentDiagram?.id + 1}`}
                                color="#353535"
                                strokeWidth={1}
                                gridBreak="90%"
                                path="grid"
